...
const fruitClassFunctions = (fruitClass, fruitConfig) => ({
  async getCurrentlyDeployed () {
    ...
    for (const fruitName of Object.keys(ret)) {
      ...
      if (missingProps.length) {
        throw new Error('getCurrentlyDeployed return is missing props: ' + JSON.stringify(missingProps) + forFruitMsg)
      }

      const extraProps = Object.keys(fruitInfo)
        .filter(prop => !(fruitConfig.props.includes(prop) || fruitConfig.postDeployProps.includes(prop)))
...
const Stateless = (fruitConfig) => {
  ...
  for (const mandatoryFunction of ['getCurrentlyDeployed', 'deploy']) {
    ensureConfig(fruitConfig[mandatoryFunction], 'Missing ' + mandatoryFunction + ' function')
  }
  Object.assign(
    fruitClass,
