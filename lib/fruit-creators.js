...
class StatelessFruit {
  ...
  async getCurrentProps () {
    ...
    const ret = await this._getCurrentProps()

    const missingKeys = this.props.filter(prop => !(prop in ret))
...
const Stateless = (fruitConfig = {}) => {
  ...
  if (fruitConfig.props && fruitConfig.props.length && !fruitConfig.getCurrentProps) {
